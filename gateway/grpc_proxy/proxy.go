...
func (p *GrpcProxy) HandleGrpcRequest(w http.ResponseWriter, r *http.Request) {
	...
	if !p.ValidateGrpcAddr(grpcAddr) {
		logger.GetLogger().Warn().Str("address", grpcAddr).Msg("gRPC address not in whitelist")
	...
	if err != nil {
		logger.GetLogger().Error().Err(err).Msg("failed to parse gRPC request")
	...
	if err != nil {
		logger.GetLogger().Error().Err(err).Msg("gRPC call failed")
...
func (p *GrpcProxy) getConnection(addr string) (*grpc.ClientConn, error) {
	...
	logger.GetLogger().Info().Str("address", addr).Msg("created new gRPC connection")
...
func (p *GrpcProxy) writeGrpcResponse(w http.ResponseWriter, resp *GrpcResponse) {
	...
	if err := json.NewEncoder(w).Encode(resp); err != nil {
		logger.GetLogger().Error().Err(err).Msg("failed to encode gRPC response")
	}
}
...
func (p *GrpcProxy) Close() {
	...
	for addr, conn := range p.connPool {
		if err := conn.Close(); err != nil {
			logger.GetLogger().Error().Err(err).Str("address", addr).Msg("failed to close gRPC connection")
		}
	}
